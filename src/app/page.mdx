...
{ 
  ...
  "status": "nulla", 
  "reassignmentDate": "02-09-2015",
  "occurrenceOfStatusDate": "02-07-2015"
}
```
...
- `status` *(string, required)* → Current status of the phone number. Allowed values:
  ...
  - `BLACKLISTED`
  - `NORMAL`
- `reassignmentDate` *(string, optional)* → Date the number was reassigned, format `DD-MM-YYYY`. Required if `status = SWAPPED OR CHURNED`
- `occurrenceOfStatusDate` *(string, required)* → Date when the status became effective, format `DD-MM-YYYY`
...
},
{
  "status": "CHURNED,SWAPPED,BLACKLISTED,NORMAL,NULL",
  "uid": "quis labore",
  "reassignmentDate": "02-09-2013",
  "occurrenceOfStatusDate": "06-09-2014"
}
```
...
- **data (array)** → Contains verification details for one or more phone numbers
  ...
  - **reassignmentDate (string, date)** → Date when the number was reassigned (if applicable). Format: `DD-MM-YYYY`
  - **verificationTimestamp (string, timestamp)** → Exact date and time the verification was performed. Format: ISO 8601 (`DD-MM-YYYY THH:MM:SS.sssZ`)
  - **occurrenceOfStatusDate (string, date)** → Date when the status event occurred (e.g., churn date, blacklist date). Format: `DD-MM-YYYY`

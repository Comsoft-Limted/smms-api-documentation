...
export const metadata = {
  ...
  description:
    'Learn everything there is to know about the TIRMS API and integrate TIRMS into your product.',
}

<HeroPattern />

# Prerequisites {{ className: 'text-4xl'}}
Before integrating with the TIRMS API, ensure the following requirements are in place: {{ className: 'lead' }}
...
- An administrator from your organization must log into the TIRMS portal to generate an API key.
...
- If you intend to use the bulk verification endpoint, set up a secure and accessible webhook URL to receive asynchronous results. The webhook can be configured via the TIRMS platform.
...
The **Telecom Identity Risk Management System (TIRMS)** API provides a centralized and secure interface for verifying and updating the status of mobile phone numbers across participating platforms. It supports fraud prevention, regulatory compliance, and the integrity of customer data. {{ className: 'lead' }}

## User Categories {{className:'lead'}}

**Telecom Operators** – Authorized to update the status of phone numbers in the TIRMS database. {{className: 'lead'}}
...
## Bulk Phone Number Verification – Responses

### 1. Request Accepted

When you submit numbers, the API confirms receipt:

```json
{
  "status": "Success",
  "data": {
    "message": "Bulk verification request accepted with 3 phone numbers. Track progress at: http://.../batch/96635c1a/verification",
    "link": "http://.../batch/96635c1a/verification"
  },
  "statusCode": "200"
}
```

- **status**: outcome of the request (Success if accepted)
- **data.message**: confirmation text
- **data.link**: URL to check results later
- **statusCode**: HTTP code (200 = accepted)

### 2. Verification Results

When you check the link, you get details for each number:
  ...
  "data": [
    {
      ...
      "status": "NORMAL",
      "requestId": "96635c1a",
      "reassignmentDate": "2025-08-15",
      "verificationTimestamp": "2025-08-15T12:34:56.789Z",
      "occurrenceOfStatusDate": "2025-08-14",
      "mobileNetwork": "MTN"
    }
  ],
  "statusCode": "200"
...
- **msisdn**: phone number checked
- **status**: current state (BLACKLISTED, CHURNED, REASSIGNED, SWAPPED, NORMAL, or NULL)
- **requestId**: batch ID
- **reassignmentDate**: date number was reassigned (if any)
- **verificationTimestamp**: when the check was done
- **occurrenceOfStatusDate**: date of the status event
- **mobileNetwork**: operator (e.g., MTN)
...
TIRMS uses standard HTTP status codes. Below are common responses: {{ className: 'lead' }}
